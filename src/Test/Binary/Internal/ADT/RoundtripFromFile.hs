...
{-# LANGUAGE RankNTypes #-}
module Test.Binary.Internal.ADT.RoundtripFromFile where
...
import           Test.Binary.Internal.ADT.GoldenSpecs
...
roundtripADTFromFile :: forall a. 
  (Typeable a, ToADTArbitrary a, Eq a, Show a, Binary.Binary a)
...
roundtripADTFromFile settings proxy = do
  ...
  describe ("Binary encoding of " ++ typeName) $
    mapM_ (testConstructorRoundtrip settings moduleName typeName) constructors

testConstructorRoundtrip :: forall a. (Eq a, Show a, Binary.Binary a, ToADTArbitrary a) =>
...
testConstructorRoundtrip Settings{..} moduleName typeName cap = do
  it ("produces the same encoding as is found in " ++ goldenFile) $ do
    ...
    if fileExist then
      Test.Binary.Internal.RoundtripFromFile.runTestFromFile (Proxy @a) goldenFile 
        `catches` 
          [ 
